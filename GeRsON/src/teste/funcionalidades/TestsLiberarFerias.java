...
public class TestsLiberarFerias {
	...
	@Test
	//teste para verificar o link de Visualizar pedidos de f√©rias
	public void testLinkVisualizarFerias() throws IOException, SAXException {
		...
		assertEquals(this.link.getText(), "Visualizar pedidos de fÈrias");
		this.link.click();
		
		this.resp = this.wc.getCurrentPage();
		assertTrue(this.resp.getText().contains("Pedidos de FÈrias"));
		assertTrue(this.resp.getText().contains("Dados para pesquisa"));
	}
	
	@Test
	//teste para aceitar as f√©rias do funcion√°rio
